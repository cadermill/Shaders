Shader "Custom/CelShader"
{
    Properties
    {
        ...
        _EdgeShadowAttenuation("Edge Shadow Attenuation", Float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            */
            ...
            // cel shaded lighting
            float3 GetLighting(float3 normal, float3 view, float4 shadow)
            {
                Light light = GetMainLight(shadow); // gets the main light in the scene

                float attenuation = light.shadowAttenuation;
                attenuation = smoothstep(0.0, _EdgeShadowAttenuation, attenuation); // smoothstep for cel shaded look
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 light = GetLighting(normal, view, shadow);

                half4 color = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv) * _BaseColor;
                return half4(color.rgb * light, color.a); // multiply the light by the base color (light is vec3 while col is vec4)
            }
            ENDHLSL
        }
        Pass
